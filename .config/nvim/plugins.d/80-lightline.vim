...
function! GetErrors()
	let l:all_errors = luaeval("vim.inspect(require(\"diagnostic.util\").buf_diagnostics_count(\"Error\"))")
	return l:all_errors == 0 ? '' : printf(g:indicator_errors . '%d', all_errors)
endfunction

function! GetWarnings()
	let l:all_warn = luaeval("vim.inspect(require(\"diagnostic.util\").buf_diagnostics_count(\"Warning\"))")
	return l:all_warn == 0 ? '' : printf(g:indicator_warnings . '%d', all_warn)
endfunction

function! GetInformations()
	let l:all_info = luaeval("vim.inspect(require(\"diagnostic.util\").buf_diagnostics_count(\"Information\"))")
